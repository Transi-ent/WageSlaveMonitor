...
| **控制台登录** | 可选环境变量 `CONSOLE_PASSWORD`，浏览器 Cookie 会话；若配置 `AUTH_TOKEN`，API 仍可使用 `Authorization: Bearer`。 |
...
默认监听 `:8080`，数据目录 `./data`（可通过 `DATA_DIR` 修改）。
...
| `DB_PATH` | SQLite 文件路径（默认 `$DATA_DIR/meta.db`）。 |
| `AUTH_TOKEN` | 若设置，客户端上传等 API 需携带 `Authorization: Bearer <token>`。 |
| `CONSOLE_PASSWORD` | 若设置，访问控制台需先打开 `/console/login` 登录。 |
| `DEFAULT_CAPTURE_INTERVAL_SECONDS` | 新客户端默认截屏间隔秒数（默认 `30`）。 |
| `RETENTION_DAYS` | 自动删除早于该天数的截图（默认 `14`）。 |

**Linux 生产部署**（编译、`systemd`、环境文件）：见 [docs/deployment-linux.md](docs/deployment-linux.md)。
...
2. 若设置了 `CONSOLE_PASSWORD`，会跳转至 `/console/login`，输入该密码登录。
